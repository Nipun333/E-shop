"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Seller = void 0;
const class_validator_1 = require("class-validator");
class Seller {
}
__decorate([
    (0, class_validator_1.IsNotEmpty)({ message: "Id can't be empty!" }),
    (0, class_validator_1.IsNumber)(),
    __metadata("design:type", Number)
], Seller.prototype, "id", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({ message: "Name can't be empty!" }),
    (0, class_validator_1.IsString)({ message: "Name should be a string" }),
    (0, class_validator_1.MinLength)(3, { message: "Name is too short" }),
    (0, class_validator_1.MaxLength)(10, { message: "Name is too big!" }),
    __metadata("design:type", String)
], Seller.prototype, "name", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({ message: "Email can't be empty!" }),
    (0, class_validator_1.IsEmail)(),
    __metadata("design:type", String)
], Seller.prototype, "email", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({ message: "Password can't be empty!" }),
    __metadata("design:type", String)
], Seller.prototype, "password", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({ message: "Address can't be empty!" }),
    __metadata("design:type", String)
], Seller.prototype, "address", void 0);
exports.Seller = Seller;
//# sourceMappingURL=seller.dto.js.map